...
> **课程大作业**：在三维有障碍、复合风场环境下，使用参数共享 PPO 训练多架无人机协同搜索隐藏目标。
...
完整报告（方法设计、奖励函数、消融实验、轨迹分析）见 [`outline.md`](outline.md)。

报告纲要：
- **第 1 节** – 问题建模：Dec-POMDP、三维质点运动学、复合风场、分区障碍生成。
- **第 2 节** – 算法设计：独立 PPO + 参数共享、七项塑形奖励、9 维连续动作空间。
- **第 3 节** – 仿真与评估：收敛分析、轨迹可视化、阵风韧性验证、A2C 基线对比。
- **第 4 节** – 结论与展望：通信增强、动态目标、sim-to-real 迁移。
...
本项目仅供学术交流与教育使用，转载请注明出处。

---

*报告完成日期：2026/06/23*  
*实验环境：rl_env (conda)，Python 3.10，Stable-Baselines3 2.5.0，NumPy 1.26*  
*硬件平台：CPU (Intel/AMD)，无 GPU 依赖*
